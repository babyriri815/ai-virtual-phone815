...
/**
 * Accept either the raw MUSIC_U value, `MUSIC_U=value`, or a copied cookie
 * string. Only MUSIC_U is retained; unrelated browser cookies are discarded.
 */
export function normalizeNeteaseMusicU(input: string): string | null {
    const trimmed = input.trim();
    if (!trimmed) return null;

    const match = trimmed.match(/(?:^|;\s*)MUSIC_U\s*=\s*([^;]+)/i);
    const token = (match?.[1] || (!/[;\s]/.test(trimmed) ? trimmed : "")).trim();
    if (!token || token.length < 16 || token.length > 4096 || /\s/.test(token)) return null;
    return `MUSIC_U=${token}`;
}

/**
 * Keep authenticated Netease requests out of URL history and Vercel request
 * URLs. NeteaseCloudMusicApi accepts the same parameters in a JSON POST body.
 * Anonymous requests remain GETs so QR generation behaviour stays unchanged.
 */
async function fetchNetease(url: string, init?: RequestInit): Promise<Response> {
    const cookie = loadNeteaseCookie();
    try {
        const parsed = new URL(url);
        if (!parsed.searchParams.has("realIP")) parsed.searchParams.set("realIP", NETEASE_REAL_IP);
        if (!cookie) return fetch(parsed.toString(), init);
        ...
        return fetch(parsed.toString(), {
            ...
            method: "POST",
            headers,
            body: JSON.stringify({ ...parameters, cookie }),
        });
    } catch {
        // Config validation rejects malformed URLs elsewhere. Fall back to a
        // credential-free request rather than exposing MUSIC_U in a URL.
...
export async function searchNetease(query: string, limit = 20): Promise<NeteaseSearchResult[]> {
    ...
    if (!base) return [];
    try {
        const resp = await fetchNetease(`${base}/cloudsearch?keywords=${encodeURIComponent(query)}&limit=${limit}`);
...
export async function getNeteasePlayInfo(songId: number): Promise<NeteasePlayInfo> {
    ...
    try {
        const resp = await fetchNetease(`${base}/song/url?id=${songId}`);
        const data = await resp.json();
        const d = data?.data?.[0];
        const url = d?.url;
        ...
        try {
            const chk = await fetchNetease(`${base}/check/music?id=${songId}&timestamp=${Date.now()}`).then(r => r.json());
...
export async function getNeteaseLyrics(songId: number): Promise<string> {
    ...
    if (!base) return "";
    try {
        const resp = await fetchNetease(`${base}/lyric?id=${songId}`);
        const data = await resp.json();
...
export async function getNeteaseSongDetail(songId: number): Promise<{ coverUrl?: string; name?: string; artists?: string; artistList?: { id: number; name: string }[]; album?: string } | null> {
    ...
    if (!base) return null;
    try {
        const resp = await fetchNetease(`${base}/song/detail?ids=${songId}`);
        const data = await resp.json();
...
export async function getQrKey(baseUrl: string): Promise<string | null> {
    try {
        ...
        const resp = await fetchNetease(`${url}/login/qr/key?timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getQrImage(baseUrl: string, key: string): Promise<string | null> {
    try {
        const url = resolveNeteaseRequestBase(baseUrl);
        const resp = await fetchNetease(`${url}/login/qr/create?key=${key}&qrimg=true&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function checkQrStatus(baseUrl: string, key: string): Promise<{ code: number; message: string; nickname?: string; cookie?: string }> {
    try {
        const url = resolveNeteaseRequestBase(baseUrl);
        const resp = await fetchNetease(`${url}/login/qr/check?key=${key}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function checkLoginStatus(baseUrl: string): Promise<{ loggedIn: boolean; nickname?: string }> {
    try {
        const url = resolveNeteaseRequestBase(baseUrl);
        const resp = await fetchNetease(`${url}/login/status?timestamp=${Date.now()}`);
        const data = await resp.json();
...
async function getLoginUid(): Promise<number | null> {
    ...
    if (!base) return null;
    try {
        const resp = await fetchNetease(`${base}/login/status?timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getUserPlaylists(): Promise<NeteasePlaylist[]> {
    ...
    if (!uid) return [];
    try {
        const resp = await fetchNetease(`${base}/user/playlist?uid=${uid}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getPlaylistTracks(playlistId: number): Promise<NeteaseSearchResult[]> {
    ...
    if (!base) return [];
    try {
        const resp = await fetchNetease(`${base}/playlist/track/all?id=${playlistId}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getDailyRecommendSongs(): Promise<NeteaseSearchResult[]> {
    ...
    if (!base) return [];
    try {
        const resp = await fetchNetease(`${base}/recommend/songs?timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getPersonalFm(): Promise<NeteaseSearchResult[]> {
    ...
    if (!base) return [];
    try {
        const resp = await fetchNetease(`${base}/personal_fm?timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getPersonalizedPlaylists(limit = 12): Promise<NeteasePlaylist[]> {
    ...
    if (!base) return [];
    try {
        const resp = await fetchNetease(`${base}/personalized?limit=${limit}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getRecommendResource(): Promise<NeteasePlaylist[]> {
    ...
    if (!base) return [];
    try {
        const resp = await fetchNetease(`${base}/recommend/resource?timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getHotSearchDetail(): Promise<NeteaseHotSearch[]> {
    ...
    if (!base) return [];
    try {
        const resp = await fetchNetease(`${base}/search/hot/detail?timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getToplists(): Promise<NeteaseToplist[]> {
    ...
    if (!base) return [];
    try {
        const resp = await fetchNetease(`${base}/toplist/detail?timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getPlaylistDetail(playlistId: number): Promise<NeteasePlaylistDetail | null> {
    ...
    if (!base) return null;
    try {
        const resp = await fetchNetease(`${base}/playlist/detail?id=${playlistId}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getSongCommentPage(songId: number, offset = 0, limit = 20, resType: NeteaseCommentResType = 0): Promise<NeteaseCommentPage> {
    ...
    try {
        ...
        const resp = await fetchNetease(`${base}/${endpoint}?id=${songId}&limit=${limit}&offset=${offset}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getFloorComments(songId: number, parentCommentId: number, limit = 20, resType: NeteaseCommentResType = 0): Promise<NeteaseComment[]> {
    ...
    if (!base) return [];
    try {
        const resp = await fetchNetease(`${base}/comment/floor?parentCommentId=${parentCommentId}&id=${songId}&type=${resType}&limit=${limit}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function postSongComment(songId: number, content: string, resType: NeteaseCommentResType = 0): Promise<{ ok: boolean; message: string }> {
    ...
    try {
        const resp = await fetchNetease(`${base}/comment?t=1&type=${resType}&id=${songId}&content=${encodeURIComponent(content)}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function subscribePlaylist(playlistId: number, subscribe: boolean): Promise<{ ok: boolean; message: string }> {
    ...
    try {
        const resp = await fetchNetease(`${base}/playlist/subscribe?t=${subscribe ? 1 : 2}&id=${playlistId}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getArtistDetail(artistId: number): Promise<NeteaseArtist | null> {
    ...
    try {
        const [detailResp, fansResp] = await Promise.all([
            fetchNetease(`${base}/artist/detail?id=${artistId}&timestamp=${Date.now()}`).then(r => r.json()),
            fetchNetease(`${base}/artist/follow/count?id=${artistId}&timestamp=${Date.now()}`).then(r => r.json()).catch(() => null),
        ]);
...
export async function getArtistTopSongs(artistId: number): Promise<NeteaseSearchResult[]> {
    ...
    if (!base) return [];
    try {
        const resp = await fetchNetease(`${base}/artist/top/song?id=${artistId}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getArtistAlbums(artistId: number, limit = 20): Promise<NeteaseAlbum[]> {
    ...
    if (!base) return [];
    try {
        const resp = await fetchNetease(`${base}/artist/album?id=${artistId}&limit=${limit}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getUserDetail(): Promise<NeteaseUserDetail | null> {
    ...
    if (!uid) return null;
    try {
        const resp = await fetchNetease(`${base}/user/detail?uid=${uid}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function getUserRecordWithCounts(type: 0 | 1 = 1): Promise<NeteasePlayRecord[]> {
    ...
    if (!uid) return [];
    try {
        const resp = await fetchNetease(`${base}/user/record?uid=${uid}&type=${type}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function addTracksToPlaylist(playlistId: number, trackIds: number[]): Promise<{ ok: boolean; message: string }> {
    ...
    if (!base) return { ok: false, message: "API 未配置" };
    try {
        const resp = await fetchNetease(`${base}/playlist/tracks?op=add&pid=${playlistId}&tracks=${trackIds.join(",")}&timestamp=${Date.now()}`);
        const data = await resp.json();
...
export async function removeTracksFromPlaylist(playlistId: number, trackIds: number[]): Promise<{ ok: boolean; message: string }> {
    ...
    if (!base) return { ok: false, message: "API 未配置" };
    try {
        const resp = await fetchNetease(`${base}/playlist/tracks?op=del&pid=${playlistId}&tracks=${trackIds.join(",")}&timestamp=${Date.now()}`);
        const data = await resp.json();
        if (data?.body?.code === 200 || data?.status === 200 || data?.code === 200) {
...
export async function testNeteaseConnection(baseUrl: string): Promise<{ ok: boolean; message: string }> {
    try {
        const url = resolveNeteaseRequestBase(baseUrl);
        const resp = await fetchNetease(`${url}/search?keywords=test&limit=1`, { signal: AbortSignal.timeout(20000) });
